        ...
        (async () => {
            ...
            try {
                ...
                if (userType === 'guardian') {
                    const confirmationCode = await AsyncStorage.getItem('confirmationCode');
                    const elderlyUserSnapshot = await firestore
                        .collection("users")
                        .where("confirmationCode", "==", confirmationCode)
                        .get();
                    if (!elderlyUserSnapshot.empty) {
                        const elderlyUserData = elderlyUserSnapshot.docs[0].data();
                        currentLocation = {
                            coords: {
                                latitude: elderlyUserData.location.latitude,
                                longitude: elderlyUserData.location.longitude
                            }
                        };
                    }
                } else {
